<?php
define('MM_BRUTE_FILE', get_template_directory().'/includes/auth/brute.txt');
...
class AntiBruteForce {
	...
	public static function bruteCheck($failed_attempt = false) {
		...
		AntiBruteForce::stringToFile(MM_BRUTE_FILE, serialize($cache));
	...
	public static function getRemainingTime () {
		$cache = unserialize(AntiBruteForce::fileToString(MM_BRUTE_FILE));
		$attempts = $cache[$_SERVER['REMOTE_ADDR']];
		$time_left = round(($attempts[0] + MM_BRUTE_WINDOW - time())/60);
		return $time_left;
	}

	public static function getTrys(){
		$cache = unserialize(AntiBruteForce::fileToString(MM_BRUTE_FILE));
